...
async function loadTeacherDashboard() {
  ...
  if (classList) classList.innerHTML = "Loading classes...";
  if (table) table.innerHTML = `<tr><td colspan="9">Loading students...</td></tr>`;

  if (typeof window.getTeacherDashboardData !== "function") {
    if (table) table.innerHTML = `<tr><td colspan="9">Firebase dashboard function not ready.</td></tr>`;
    return;
  }
  ...
  if (!result.ok) {
    ...
    if (table) table.innerHTML = `<tr><td colspan="9">${result.message}</td></tr>`;
    return;
  }
...
function renderTeacherStudents(students) {
  ...
  const filtered = students.filter(s => {
    const combined = [
      s.username, s.fullName, s.studentClass, s.className, s.schoolName, s.stateName
    ].join(" ").toLowerCase();
    return !keyword || combined.includes(keyword);
  });

  if (!filtered.length) {
    table.innerHTML = `<tr><td colspan="9">No student data found.</td></tr>`;
    return;
  }
  ...
  table.innerHTML = filtered.map(s => {
    ...
    let bestText = last.sifir || "-";
    if (recordEntries.length) {
      const best = recordEntries
        .filter(([_, r]) => r && r.bestTime)
        .sort((a,b) => Number(a[1].bestTime) - Number(b[1].bestTime))[0];
      if (best) bestText = `Sifir ${best[0]} (${best[1].bestTime}s)`;
    }
    const completed = Object.values(s.progress || {}).reduce((a,b) => a + Number(b || 0), 0);
    ...
    return `
      <tr>
        ...
        <td>${s.schoolName || "-"}</td>
        <td>${s.stateName || "-"}</td>
        <td>${Number(s.coins || last.coins || 0)}</td>
        <td>${Number(s.xp || last.xpAwarded || 0)}</td>
        <td>${bestText}</td>
        <td>${last.time ? last.time + "s" : "-"}</td>
        <td>${completed}</td>
      </tr>
    `;
  }).join("");
