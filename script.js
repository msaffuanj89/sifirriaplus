...
function nextChallengeQuestion() {
  ...
  if ($("challengeLevelText")) $("challengeLevelText").textContent = "Level " + challengeState.level;
...
async function loadTeacherDashboard() {
  ...
  if (table) table.innerHTML = `<tr><td colspan="13">Loading fast student summary...</td></tr>`;

  if (typeof window.getTeacherDashboardData !== "function") {
    if (table) table.innerHTML = `<tr><td colspan="13">Firebase dashboard function not ready.</td></tr>`;
    return;
  }
  ...
  if (!result.ok) {
    ...
    if (table) table.innerHTML = `<tr><td colspan="13">${result.message}</td></tr>`;
    return;
  }
...
function normalizeAdminUser(user) {
  ...
  return {
    ...
    totalAttempts,
    activeScore,
    isLearningActive: totalAttempts > 0 || challengeAttempts > 0 || activeDaysCount > 0 || activeScore > 0
  };
}
...
function renderAdminDashboard() {
  ...
  if ($("adminStudentTable")) {
    ...
    `).join("") : `<tr><td colspan="11">No active student data found.</td></tr>`;
  }
  ...
  if ($("adminUserTable")) {
    ...
    `).join("") : `<tr><td colspan="11">No user data found.</td></tr>`;
  }
}

async function loadAdminDashboard() {
  if ($("adminUserTable")) $("adminUserTable").innerHTML = `<tr><td colspan="11">Loading admin data...</td></tr>`;
  if ($("adminSchoolTable")) $("adminSchoolTable").innerHTML = `<tr><td colspan="8">Loading admin data...</td></tr>`;
  if ($("adminStudentTable")) $("adminStudentTable").innerHTML = `<tr><td colspan="11">Loading admin data...</td></tr>`;
  ...
  if (typeof window.getAdminDashboardData !== "function") {
    if ($("adminUserTable")) $("adminUserTable").innerHTML = `<tr><td colspan="11">Firebase admin function not ready.</td></tr>`;
    return;
  }
  ...
  if (!result.ok) {
    ...
    if ($("adminUserTable")) $("adminUserTable").innerHTML = `<tr><td colspan="11">${message}</td></tr>`;
    if ($("adminSchoolTable")) $("adminSchoolTable").innerHTML = `<tr><td colspan="8">${message}</td></tr>`;
    if ($("adminStudentTable")) $("adminStudentTable").innerHTML = `<tr><td colspan="11">${message}</td></tr>`;
...
function buildFastStudentSummary(s) {
  ...
  const activeScore = coins + (activeDays.size * 10) + (streak * 5) + (sifirKeys.length * 15) + (challengeSummary.correct * 4);
...
function renderTeacherStudents(students) {
  ...
  if (!summary.length) {
    table.innerHTML = `<tr><td colspan="13">No student summary found.</td></tr>`;
    return;
  }
