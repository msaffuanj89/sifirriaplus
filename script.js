...
$("authForm").onsubmit = async (e) => {
      ...
      if (authMode === "signup") {
        ...
        const result = await window.registerAccountFirebase(user, pass);
        ...
        localStorage.setItem("user_profile_" + user, JSON.stringify({ username:user, password: pass }));
        $("loginError").textContent = result.message;
        setAuthMode("login");
        return;
      }
      ...
      if (typeof window.loginAccountFirebase === "function") {
        ...
        if (result.ok) {
          currentUser = user;
          applyFirebaseAccountData(result.account || {});
          login(user, result.username || user);
          return;
        }

        const profileRaw = localStorage.getItem("user_profile_" + user);
        if (profileRaw) {
